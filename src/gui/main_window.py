...
from ..render import effects as _render_effects
from ..render import transitions as _render_transitions
from ..render import images as _render_images
from ..render import process as _render_process
from ..render import codec as _render_codec
from ..render import audio as _render_audio
from ..render import context as _render_context
from ..render import controls as _render_controls
from ..render import watermark as _render_watermark
from ..render import encoder as _render_encoder
from ..render import compositor as _render_compositor
from ..render import postprocess as _render_postprocess
from ..render import job as _render_job
from ..render.plan import VIDEO_EFFECTS, compute_video_frame_plan
...
class ImageToVideoTab:
    ...
    # --- 渲染内核接口：按名读取界面侧状态 + 组装上下文 ---

    def _render_read(self, name, default=None):
        """按名读取界面侧配置/状态，供渲染内核使用。"""
    ...
    def _render_write(self, name, value):
        """按名写回界面侧状态：Tk 变量走 .set()，普通属性直接赋值。"""
        cur = getattr(self, name, None)
        setter = getattr(cur, "set", None)
        if callable(setter):
            setter(value)
        else:
            setattr(self, name, value)

    def _render_ctx(self):
        """组装渲染内核所需的界面侧上下文。"""
        codec_var = getattr(self, "codec_var", None)
        return _render_context.RenderContext(
            codec_provider=(lambda: codec_var.get()) if codec_var is not None else None,
            runtime_probe=getattr(self, "_runtime_probe", None),
            ffmpeg_available=bool(getattr(self, "ffmpeg_available", False)),
            ffmpeg_executable=getattr(self, "ffmpeg_executable", None),
            ffprobe_executable=getattr(self, "ffprobe_executable", None),
            startupinfo=getattr(self, "startupinfo", None),
            notify=self.update_status,
            fps_provider=lambda: self.fps.get(),
            probe_cache=getattr(self, "_codec_probe_cache", None),
            read=self._render_read,
            set_value=self._render_write,
            progress=self.update_progress,
            reset_progress=self.reset_progress,
            reset_overall_progress=self.reset_overall_progress,
            cancel_provider=lambda: getattr(self, "cancel_requested", False),
            pause_event=getattr(self, "pause_event", None),
            turbo_accelerator=getattr(self, "turbo_accelerator", None),
            transition_engine=getattr(self, "transition_engine", None),
        )

    # --- 编码/容器解析：实现已抽到渲染内核 src.render.codec ---

    def _codec_context(self):
        """组装编码解析所需的界面侧上下文。"""
        codec_var = getattr(self, "codec_var", None)
        return _render_codec.CodecContext(
            codec_provider=(lambda: codec_var.get()) if codec_var is not None else None,
            runtime_probe=getattr(self, "_runtime_probe", None),
            ffmpeg_available=bool(getattr(self, "ffmpeg_available", False)),
            ffmpeg_executable=getattr(self, "ffmpeg_executable", None),
    ...
    def check_codec_availability(self, codec, force_recheck=False):
        """检查编码器可用性（实现见渲染内核 src.render.codec）"""
        return _render_codec.check_codec_availability(codec, force_recheck, self._codec_context())
    ...
    def normalize_path(self, path):
        """标准化路径（实现见渲染内核 src.render.images）"""
        return _render_images.normalize_path(path, self.update_status)
    ...
    # --- 图片装载与基础处理：实现已抽到渲染内核 src.render.images ---

    def get_images_list(self, input_dir, limit_count=None, selection_mode="随机选择"):
        """获取图片列表（实现见渲染内核 src.render.images）"""
        return _render_images.get_images_list(
            input_dir, limit_count, selection_mode, self.update_status, self.turbo_accelerator
        )

    def natural_sort_key(self, filename):
        return _render_images.natural_sort_key(filename)

    def sort_images_naturally(self, image_paths):
        return _render_images.sort_images_naturally(image_paths, self.update_status)

    def safe_read_image(self, img_path):
        return _render_images.safe_read_image(img_path, self.update_status, self.turbo_accelerator)

    def resize_with_aspect_ratio(self, img, target_width, target_height):
        return _render_images.resize_with_aspect_ratio(img, target_width, target_height)

    def _center_crop(self, img, target_width, target_height):
        """从中心裁剪到目标尺寸（实现见渲染内核 src.render.effects）"""
        return _render_effects._center_crop(img, target_width, target_height)
    ...
    def apply_single_image_effect(self, img, effect_type, time_sec, duration_sec, intensity=100.0, speed=1.0):
        """单图视频特效（实现见渲染内核 src.render.effects）"""
        return _render_effects.apply_single_image_effect(
            img, effect_type, time_sec, duration_sec, intensity, speed
        )

    # --- 转场渲染：实现已抽到渲染内核 src.render.transitions ---

    def apply_fade_transition(self, img1, img2, frame_idx, total_frames):
        return _render_transitions.apply_fade_transition(img1, img2, frame_idx, total_frames)

    def apply_blinds_transition(self, img1, img2, frame_idx, total_frames):
        return _render_transitions.apply_blinds_transition(img1, img2, frame_idx, total_frames)

    def apply_slide_transition(self, img1, img2, frame_idx, total_frames):
        return _render_transitions.apply_slide_transition(img1, img2, frame_idx, total_frames)

    def apply_dissolve_transition(self, img1, img2, frame_idx, total_frames):
        return _render_transitions.apply_dissolve_transition(img1, img2, frame_idx, total_frames)

    def apply_wipe_transition(self, img1, img2, frame_idx, total_frames):
        return _render_transitions.apply_wipe_transition(img1, img2, frame_idx, total_frames)

    def apply_transition(self, img1, img2, video_writer, num_frames, transition_type="淡入淡出"):
        """转场合成（实现见渲染内核 src.render.transitions）"""
        return _render_transitions.apply_transition(
            img1, img2, video_writer, num_frames, transition_type, self.transition_engine
        )

    # --- 编码/容器解析转发（实现见渲染内核 src.render.codec）---

    def get_h264_codec(self):
        return _render_codec.get_h264_codec(self._codec_context())

    def _get_selected_codec_name(self):
        return _render_codec.get_selected_codec_name(self._codec_context())

    def _resolve_cv_fourcc(self):
        return _render_codec.resolve_cv_fourcc(self._codec_context())

    def _resolve_processing_fourcc(self):
        return _render_codec.resolve_processing_fourcc(self._codec_context())

    def _get_ffmpeg_vcodec(self):
        return _render_codec.get_ffmpeg_vcodec(self._codec_context())

    def _get_strict_ffmpeg_vcodec_for_output(self, output_path):
        return _render_codec.get_strict_ffmpeg_vcodec_for_output(output_path, self._codec_context())

    def _get_output_extension(self, output_path):
        return _render_codec.get_output_extension(output_path, self._codec_context())

    def _build_temp_output_path(self, output_path, suffix):
        return _render_codec.build_temp_output_path(output_path, suffix, self._codec_context())

    def _get_ffmpeg_muxer_for_output(self, output_path):
        return _render_codec.get_ffmpeg_muxer_for_output(output_path, self._codec_context())

    def _get_container_compatible_vcodecs(self, output_path, preferred_codec=None):
        return _render_codec.get_container_compatible_vcodecs(
            output_path, preferred_codec, self._codec_context()
        )

    def _get_container_compatible_acodec(self, output_path):
        return _render_codec.get_container_compatible_acodec(output_path, self._codec_context())

    def _get_fallback_processing_fourcc(self):
        return _render_codec.get_fallback_processing_fourcc(self._codec_context())

    def _reencode_video_to_selected_codec(self, src_path, dst_path, log_func=None):
        return _render_codec.reencode_video_to_selected_codec(
            src_path, dst_path, log_func, self._codec_context()
        )

    def _log_output_probe(self, output_path, log_func=None):
        return _render_codec.log_output_probe(output_path, log_func, self._codec_context())

    def create_video(self, images, output_path, duration, fps, width=1920, height=1080, apply_image_watermark=False, watermark_path=None, watermark_position='右下', resize_mode='适应', maintain_aspect=True, transition_frames=15, transition_type='淡入淡出', watermark_opacity=0.5, watermark_size=20, video_effect_type_override=None):
        return _render_compositor.create_video(self._render_ctx(), images, output_path, duration, fps, width, height, apply_image_watermark, watermark_path, watermark_position, resize_mode, maintain_aspect, transition_frames, transition_type, watermark_opacity, watermark_size, video_effect_type_override)

    def create_video_turbo_enhanced(self, images, output_path, duration, fps, width=1920, height=1080, apply_image_watermark=False, watermark_path=None, watermark_position='右下', resize_mode='适应', maintain_aspect=True, transition_frames=15, transition_type='淡入淡出', watermark_opacity=0.5, watermark_size=20, video_effect_type_override=None):
        return _render_compositor.create_video_turbo_enhanced(self._render_ctx(), images, output_path, duration, fps, width, height, apply_image_watermark, watermark_path, watermark_position, resize_mode, maintain_aspect, transition_frames, transition_type, watermark_opacity, watermark_size, video_effect_type_override)

    def _turbo_preprocess_image(self, img_path, width, height, resize_mode, maintain_aspect, apply_watermark, watermark_layers, watermark_position, watermark_opacity, watermark_size, path_index_map=None):
        return _render_compositor.turbo_preprocess_image(self._render_ctx(), img_path, width, height, resize_mode, maintain_aspect, apply_watermark, watermark_layers, watermark_position, watermark_opacity, watermark_size, path_index_map)

    def _turbo_write_transition_frames(self, video_writer, img1, img2, num_frames, transition_type):
        return _render_compositor.turbo_write_transition_frames(self._render_ctx(), video_writer, img1, img2, num_frames, transition_type)

    def add_audio_with_ffmpeg(self, video_path, audio_file, volume=0.5):
        return _render_audio.add_audio_with_ffmpeg(video_path, audio_file, volume, self._render_ctx())

    def verify_watermark_file(self):
        return _render_watermark.verify_watermark_file(self._render_ctx())
    ...
    def get_audio_files(self, directory):
        return _render_audio.get_audio_files(directory, self._render_ctx())

    def get_watermark_files(self, directory):
        return _render_watermark.get_watermark_files(self._render_ctx(), directory)
    ...
    def get_enabled_transitions(self):
        return _render_job.get_enabled_transitions(self._render_ctx())

    def _build_random_transition_plan(self, video_count):
        return _render_job.build_random_transition_plan(self._render_ctx(), video_count)

    def get_enabled_video_effects(self):
        return _render_job.get_enabled_video_effects(self._render_ctx())
    ...
    # --- 控制原语：实现已抽到渲染内核 src.render.controls ---

    def _wait_for_processing_control(self, sleep_sec=0.03):
        return _render_controls.wait_for_processing_control(sleep_sec, self._render_ctx())

    def resize_image(self, img, target_w, target_h, mode='适应', keep_aspect=True):
        return _render_images.resize_image(img, target_w, target_h, mode, keep_aspect)

    def _ensure_even_frame(self, frame):
        return _render_process.ensure_even_frame(frame)

    def _log_pipeline_stage(self, stage, message, log_func=None):
        return _render_process.log_pipeline_stage(
            stage, message, log_func, getattr(self, "_pipeline_log_file", None)
        )

    def _run_process_with_retry(self, cmd, stage="PROC", timeout_sec=300, retries=1, log_func=None):
        return _render_process.run_process_with_retry(
            cmd, stage, timeout_sec, retries, log_func, getattr(self, "startupinfo", None)
        )

    @staticmethod
    def _safe_replace_file(src_path, dst_path):
        return _render_process.safe_replace_file(src_path, dst_path)

    def _probe_video_meta(self, video_path):
        return _render_process.probe_video_meta(
            video_path,
            getattr(self, "ffprobe_executable", None),
            getattr(self, "ffmpeg_executable", None),
            getattr(self, "startupinfo", None),
            lambda: self.fps.get(),
        )

    def _calc_overlay_geometry(self, main_w, main_h, wm_w, wm_h, size_mode, scale_value, position):
        return _render_watermark.calc_overlay_geometry(self._render_ctx(), main_w, main_h, wm_w, wm_h, size_mode, scale_value, position)

    def _select_video_watermark_file(self, log_func=None):
        return _render_watermark.select_video_watermark_file(self._render_ctx(), log_func)

    def _resolve_bgm_candidates(self):
        return _render_audio.resolve_bgm_candidates(self._render_ctx())

    def _select_bgm_file(self):
        return _render_audio.select_bgm_file(self._render_ctx())

    def create_video_with_ffmpeg(self, images, output_path, duration, fps, width, height, apply_image_watermark, follow_layers, fixed_layers, watermark_position, transition_frames, transition_type, watermark_opacity, watermark_size, target_bitrate, log_func, video_effect_type_override=None, preprocessed_images=None):
        return _render_encoder.create_video_with_ffmpeg(self._render_ctx(), images, output_path, duration, fps, width, height, apply_image_watermark, follow_layers, fixed_layers, watermark_position, transition_frames, transition_type, watermark_opacity, watermark_size, target_bitrate, log_func, video_effect_type_override, preprocessed_images)

    def _has_postprocess_work(self, fixed_layers=None):
        return _render_encoder.has_postprocess_work(self._render_ctx(), fixed_layers)

    def _get_video_watermark_blend_mode(self):
        return _render_watermark.get_video_watermark_blend_mode(self._render_ctx())

    def _get_video_watermark_alpha(self, blend_mode):
        return _render_watermark.get_video_watermark_alpha(self._render_ctx(), blend_mode)

    def _get_ffmpeg_blend_mode(self, blend_mode):
        return _render_watermark.get_ffmpeg_blend_mode(self._render_ctx(), blend_mode)

    def _collect_fixed_layer_specs_for_ffmpeg(self):
        return _render_watermark.collect_fixed_layer_specs_for_ffmpeg(self._render_ctx())

    def _run_unified_postprocess_ffmpeg(self, output_path, fixed_layers, watermark_position, log_func=None):
        return _render_postprocess.run_unified_postprocess_ffmpeg(self._render_ctx(), output_path, fixed_layers, watermark_position, log_func)

    def _run_fixed_layers_ffmpeg_only(self, output_path, fixed_layers, log_func=None):
        return _render_postprocess.run_fixed_layers_ffmpeg_only(self._render_ctx(), output_path, fixed_layers, log_func)

    def _postprocess_video_output(self, output_path, fixed_layers, watermark_position, log_func=None, pipeline_start_time=None):
        return _render_postprocess.postprocess_video_output(self._render_ctx(), output_path, fixed_layers, watermark_position, log_func, pipeline_start_time)
    ...
    def _maybe_realtime_cleanup(self, force: bool = False):
        self._last_cache_cleanup = _render_controls.maybe_realtime_cleanup(
            getattr(self, "turbo_accelerator", None),
            getattr(self, "transition_engine", None),
            getattr(self, "_last_cache_cleanup", 0.0),
            getattr(self, "_cache_cleanup_interval", 2.0),
            force,
        )
    ...
    def apply_blend_mode(self, background, foreground, mode='正常', alpha=0.5):
        return _render_watermark.apply_blend_mode(self._render_ctx(), background, foreground, mode, alpha)

    def _safe_read_image_with_alpha(self, img_path):
        return _render_watermark.safe_read_image_with_alpha(self._render_ctx(), img_path)

    def _normalize_watermark_layers(self):
        return _render_watermark.normalize_watermark_layers(self._render_ctx())

    def _get_image_files_in_dir(self, directory):
        return _render_watermark.get_image_files_in_dir(self._render_ctx(), directory)

    def _prepare_image_watermark_layers(self):
        return _render_watermark.prepare_image_watermark_layers(self._render_ctx())

    def _apply_image_watermark_layer(self, image, layer, image_index=0):
        return _render_watermark.apply_image_watermark_layer(self._render_ctx(), image, layer, image_index)

    def apply_image_watermark_layers(self, image, layers, image_index=0):
        return _render_watermark.apply_image_watermark_layers(self._render_ctx(), image, layers, image_index)

    def add_fixed_image_watermarks_to_video(self, input_path, layers):
        return _render_watermark.add_fixed_image_watermarks_to_video(self._render_ctx(), input_path, layers)

    def _apply_ffmpeg_animated_overlays_stage(self, video_path: str):
        return _render_watermark.apply_ffmpeg_animated_overlays_stage(self._render_ctx(), video_path)

    def add_video_watermark(self, main_video_path, watermark_path, output_path, position='右下', match_method='循环'):
        return _render_watermark.add_video_watermark(self._render_ctx(), main_video_path, watermark_path, output_path, position, match_method)

    def resize_image_hq(self, image, target_width, target_height, resize_mode='适应', maintain_aspect=True):
        return _render_watermark.resize_image_hq(self._render_ctx(), image, target_width, target_height, resize_mode, maintain_aspect)

    def create_single_image_video(self, image_path, watermark_video_path, output_path, width, height, position='右下', match_method='循环'):
        return _render_watermark.create_single_image_video(self._render_ctx(), image_path, watermark_video_path, output_path, width, height, position, match_method)

    def blend_image_with_video_frame_hq(self, background_image, video_frame, position='右下'):
        return _render_watermark.blend_image_with_video_frame_hq(self._render_ctx(), background_image, video_frame, position)

    def blend_image_with_video_frame(self, background_image, video_frame, position='右下'):
        return _render_watermark.blend_image_with_video_frame(self._render_ctx(), background_image, video_frame, position)
    ...
    def process_videos(self):
        ...
        try:
            return _render_job.run_batch(self._render_ctx())
        finally:
            self.is_processing = False
